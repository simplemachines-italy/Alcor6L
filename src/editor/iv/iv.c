 ...
 * cole@coledd.com
 *
 * Modified by Raman Gopalan <ramangopalan@gmail.com> for eLua
 * Nov, 2012
 *
...
int iv_main(int argc, char **argv)
{
	...
	if (NULL == argv[1]) {
		...
		// raman: check if TEMP_FILE exists.
...
//			{
//				case EACCES:
//					show_error(ERROR_TYPE_FILE, "read permission denied");
//					break;
//				case ENOENT:
//					file_flags |= FILE_FLAGS_NEW;
//					break;
//				default:
//					show_error(ERROR_TYPE_FILE, "unknown file error");
//					break;
//			}
			/*
			 * r+ mode fails so may be you want to open
			 * a new file with iv?
			 */
			if (fopen(filename, "wb+") == NULL)
				show_error(ERROR_TYPE_FILE, "cannot open file.");
		}
//		if (access(filename, W_OK))
//			file_flags |= FILE_FLAGS_READONLY;
	...
	while (!(exit_program |= process_command(file_buffer))) {
		...
		if (ESC == input) {
			clear_cmd_line(cmd_line, &cmd_line_index);
			cmd_line[cmd_line_index++] = (char)input;
			// while(read(STDIN_FILENO, &cmd_line[cmd_line_index], 1) >= 0) {
			while (cmd_line[cmd_line_index] = term_getch_nt(TERM_INPUT_WAIT) >= 0) {
				// printf("n: %c\n", (char)cmd_line[cmd_line_index]);
				cmd_line_index++;
				if (cmd_line_index > MAX_CMD_LINE_SIZE)
					clear_cmd_line(cmd_line, &cmd_line_index);
				break;
...
void parse_input_cmds(struct ll_struct *file_buffer, char *cmd_line, int *cmd_line_index)
{
	...
	case ':':
		...
		case 'w':
			switch (cmd_line[2]) {
                        /* raman: rename a file with :w fooname */
                        case ' ':
                                if ('\n' == cmd_line[*cmd_line_index - 1]) {
                                        ...
                                        if (write_file(file_buffer) == 0) {
                                                printf("%c[%d;%dH%c[K\"%s\"",\
                                                ESC, win_size.ws_row, 0, ESC, filename);
                                                printf(" [new file]");
                                                printf(" written");
                                        }
                                        clear_cmd_line(cmd_line, cmd_line_index);
...
void show_error(char error_type, char *error_message)
{
	...
	printf(" error: %s\n", error_message);
	longjmp(ebuf, SHOW_ERROR_RET_VALUE);
}

/*
 * set_input_mode() - sets up terminal for single-key input for commands.
 * returns: 0 upon exit.
 */
...
//	stdin_saved = fcntl(fileno(stdin), F_GETFL);
...
//	fcntl(fileno(stdin), F_SETFL, stdin_saved);
//	printf("%c[r", ESC);
